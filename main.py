# ---------------------------------------------------------
# ✅ Mode: News Tamprakan (Normal Speed + Queue System)
# ---------------------------------------------------------
import sys
...
# 🚦 Queue Control: อนุญาตให้เรนเดอร์พร้อมกันได้แค่ 1 งาน เพื่อป้องกัน RAM เต็ม
...
def search_real_image(query, filename):
    if not query or any(x in str(query).upper() for x in ["SELECT", "INSERT", "GALLERY"]) or len(str(query)) < 3:
        return False
    try:
        with DDGS() as ddgs:
...
async def create_voice_safe(text, filename):
    try:
        ...
        try: gTTS(text=text, lang='th').save(filename)
        except: pass
...
def process_video_background(task_id, scenes, topic):
    task_id = str(task_id)
    print(f"[{task_id}] ⏳ Task received and waiting in queue...")
    
    # 🚦 เข้าคิวรอเรนเดอร์ (Semaphore)
    with render_semaphore:
        ...
        try:
            ...
            print(f"❌ Error in process_video_background [{task_id}]: {e}")
        finally:
            # ล้างไฟล์ขยะ
            for f in os.listdir():
                if str(task_id) in f:
            ...
            gc.collect()
            print(f"[{task_id}] 🏁 Task finished and resources cleared.")
...
def api_create_video():
    ...
    return jsonify({"status": "processing", "task_id": task_id, "message": "Task queued"}), 202
