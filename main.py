...
# ใช้ Key เดียวกันกับ Storage ได้เลย
GCS_KEY_JSON = os.environ.get("GCS_KEY_JSON") 
render_semaphore = threading.Semaphore(1)

# 🎙️ ระบบเสียงพากย์ Google Cloud TTS (ไทยชัด 100%)
async def generate_voice(text, filename, use_premium, task_id):
    ...
    try:
        if use_premium and GCS_KEY_JSON:
            print(f"[{task_id}] 🎙️ Google Cloud TTS (Neural2) กำลังพากย์ไทยชัดๆ...")
            ...
            # 🟢 เลือกเสียง Neural2-A (เสียงพรีเมียมภาษาไทยที่เนียนที่สุด)
            ...
            audio_config = texttospeech.AudioConfig(
                ...
                pitch=0.0,
                speaking_rate=1.05 # เร่งนิดนึงให้ดูไม่น่าเบื่อ
            )
        ...
        # Fallback: Edge-TTS
        print(f"[{task_id}] 🎙️ สลับใช้ Edge-TTS (ฟรี)...")
...
def process_master_video(task_id, qa_url, ans_url, ad_url, av_url, script_qa, script_ans, script_ad, countdown, use_premium, show_avatar):
    ...
    print(f"[{task_id}] เริ่มงานเรนเดอร์ (Google Cloud TTS Mode)")
    ...
    with render_semaphore:
        try:
            ...
            if os.path.exists("sfx_intro.mp3"):
                ...
                a1_layers.append(sfx.subclip(0, v_qa.duration) if sfx.duration > v_qa.duration else sfx)
            ...
            if os.path.exists("sfx_correct.mp3"):
                ...
                a3_layers.append(sfx_c.subclip(0, v_ans.duration) if sfx_c.duration > v_ans.duration else sfx_c)
            ...
            if has_ad:
                ...
                ad_chain.append(CompositeVideoClip([bg, fg.set_position("center")]).set_duration(v_ad.duration).set_audio(v_ad))
                final_v = concatenate_videoclips([final_v] + ad_chain)

            if os.path.exists("bgm_main.mp3"):
                final_v = final_v.set_audio(CompositeAudioClip([final_v.audio, audio_loop(AudioFileClip("bgm_main.mp3").volumex(0.12), duration=final_v.duration)]))
            ...
            requests.post(N8N_WEBHOOK_URL, json={'id': task_id, 'final_url': blob.generate_signed_url(version="v4", expiration=datetime.timedelta(hours=12)), 'status': 'success'}, timeout=20)
            print(f"[{task_id}] 🎉 สำเร็จด้วยเสียง Google!")
...
def api_render():
    ...
    threading.Thread(target=process_master_video, args=(task_id, d.get('qa_image_url'), d.get('ans_image_url'), d.get('ad_image_url'), d.get('avatar_video_url'), d.get('script_qa'), d.get('script_ans'), d.get('script_ad'), int(d.get('countdown_time', 5)), use_p, d.get('show_avatar', False))).start()
